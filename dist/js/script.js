function start_loader() {
    $('body').append('<div id="preloader"><div class="loader-holder" style="display: flex; justify-content: center; align-items: center;"><img src="' + _base_url_ +'/dist/img/loading.gif" alt="Preloader Image" title="Loading..."></div></div>')
...
$(document).ready(function() {
    ...
    $('#author-frm').submit(function(e) {
            e.preventDefault()
            start_loader()
            if ($('.err_msg').length > 0)
                $('.err_msg').remove()
            $.ajax({
                url: _base_url_ + 'classes/Login.php?f=author_login',
                method: 'POST',
                data: $(this).serialize(),
                error: err => {
                    console.log(err)
                ...
                success: function(resp) {
                    if (resp) {
                        resp = JSON.parse(resp)
                        if (resp.status == 'success') {
                            location.replace(_base_url_ + 'prf_author');
                        } else if (resp.status == 'incorrect') {
                            var _frm = $('#author-frm')
                            var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Incorrect username or password</div>"
                            _frm.prepend(_msg)
    ...
    $('#exit-frm').submit(function(e) {
            e.preventDefault()
            start_loader()
            if ($('.err_msg').length > 0)
                $('.err_msg').remove()
            $.ajax({
                url: _base_url_ + 'classes/Login.php?f=login_exit',
                method: 'POST',
                data: $(this).serialize(),
                error: err => {
                    console.log(err)
                ...
                success: function(resp) {
                    if (resp) {
                        resp = JSON.parse(resp)
                        if (resp.status == 'success') {
                            location.replace(_base_url_ + 'exit');
                        } else if (resp.status == 'incorrect') {
                            var _frm = $('#exit-frm')
                            var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Incorrect username or password</div>"
                            _frm.prepend(_msg)
    ...
    $('#prf_login').submit(function(e) {
        e.preventDefault()
        start_loader()
        if ($('.err_msg').length > 0)
            $('.err_msg').remove()
        $.ajax({
            url: _base_url_ + 'classes/Login.php?f=login_prf',
            method: 'POST',
            data: $(this).serialize(),
            error: err => {
                console.log(err)
    ...
    $('#lgn').submit(function(e) {
        ...
        if ($('.err_msg').length > 0)
            $('.err_msg').remove()
        $.ajax({
            url: _base_url_ + 'classes/Login.php?f=lgn',
            method: 'POST',
            data: $(this).serialize(),
            error: err => {
                console.log(err)
            ...
            success: function(resp) {
                if (resp) {
                    resp = JSON.parse(resp)
                    if (resp.status == 'success') {
                        location.replace(_base_url_ + 'prf_login');
                        // location.reload()
                    } else if (resp.status == 'incorrect') {
                        var _frm = $('#lgn')
                        var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Incorrect username or password</div>"
