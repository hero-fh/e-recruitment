...
// echo $total_points;
?>
<?php $apply = $conn->query("SELECT `application_id` FROM `position` where  position ='{$_settings->userdata('recommended_pos')}'")->fetch_array()[0]; ?>
...
<!-- <div class="container1 bg-transparent fixed-top">
    <?php if ($apply == 1) { ?>
...
<section class="mt-5 py-3">
    <div class="container">
        <form action="" id="take-exam">
            <div class="align-items-top mb-3">
                ...
                <div class="card card-outline card-primary shadow rounded-0">
                    <?php
                    $exam_ids = $conn->query("SELECT * FROM `exam_list` WHERE category_id = '{$id}' and status=1  and id>=39 and id<=41");
        ...
        <?php
        $app_id = $conn->query("SELECT `application_id` FROM `position` where  `position` = '{$_settings->userdata('recommended_pos')}'")->fetch_array();
        // echo  $app_id[0];
        $exams1 = $conn->query("SELECT * FROM `applicants` where  id =" . $_settings->userdata('id'));
        while ($rows = $exams1->fetch_assoc()) :
        ?>
            <?php if (isset($app_id) ? $app_id[0] == 2 : '') { ?>
            ...
            <?php
            } elseif (isset($app_id) ? $app_id[0] == 4 : '') { ?>
                <form>
                    <div class="align-items-top mb-3">
...
<script>
    ...
    $(function() {
        ...
        let isCodeExecuted = true; //====================gawing false kapag ibabalik na ang timer======================//
        ...
        function handleClick(event) {
            ...
            //=========================EXAM TIMER==========================================//
            // if (userId != 1) {
            // setTimeout(triggerButton, 90 * 60 * 1000); //90mins
            // } else if (userId == 1) {
            //     setTimeout(triggerButton, 60 * 60 * 1000); //60 mins
            // }
            ...
            if (!isCodeExecuted) {
                ...
                // console.log(userId);
                if (userId != 1) {
                    var countToDate = (+new Date) + (90 * 60 * 1000);
                } else if (userId == 1) {
                    var countToDate = (+new Date) + (60 * 60 * 1000);
                }
