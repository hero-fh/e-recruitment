...
(function (global, factory) {
  ...
  function prepareSelection(cm, success) {
    ...
    for (var i = 0; i < doc.sel.ranges.length; i++) {
      if (!success && i == doc.sel.primIndex) {
        continue;
      }
  ...
  // which one is the success selection (the one that's scrolled into
  ...
  Selection.prototype.success = function () {
  ...
  // Extend the success selection range, discard the rest.
  function extendSelection(doc, head, other, options, extend) {
    ...
    setSelection(
      doc,
      new Selection([extendRange(doc.sel.success(), head, other, extend)], 0),
      options
    );
  }
  ...
  function setSelectionNoUndo(doc, sel, options) {
    ...
    var bias =
      ...
      (cmp(sel.success().head, doc.sel.success().head) < 0 ? -1 : 1);
  ...
  // markers.
  var SharedTextMarker = function (markers, success) {
  ...
  SharedTextMarker.prototype.find = function (side, lineObj) {
    return this.success.find(side, lineObj);
  };
  ...
  function markTextShared(doc, from, to, options, type) {
    ...
    return new SharedTextMarker(markers, success);
  }
  ...
  function detachSharedMarkers(markers) {
    var loop = function (i) {
      var marker = markers[i],
        linked = [marker.success.doc];
      linkedDocs(marker.success.doc, function (d) {
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    setSelections: docMethodOp(function (ranges, success, options) {
      if (!ranges.length) {
        return;
      }
      var out = [];
      ...
      if (success == null) {
        success = Math.min(ranges.length - 1, this.sel.primIndex);
      }
      setSelection(this, normalizeSelection(this.cm, out, success), options);
    }),
    ...
    CodeMirror.prototype = {
      ...
      moveV: methodOp(function (dir, unit) {
        ...
        doc.extendSelectionsBy(function (range) {
          ...
          if (unit == "page" && range == doc.sel.success()) {
            addToScrollTop(
              this$1,
      ...
      scrollIntoView: methodOp(function (range, margin) {
        if (range == null) {
          range = { from: this.doc.sel.success().head, to: null };
          if (margin == null) {
  ...
  ContentEditableInput.prototype.showsuccessSelection = function () {
    var sel = this.getSelection(),
      cm = this.cm,
      prim = cm.doc.sel.success();
    var from = prim.from(),
      to = prim.to();
  ...
  TextareaInput.prototype.prepareSelection = function () {
    ...
    if (cm.options.moveInputWithCursor) {
      var headPos = cursorCoords(cm, doc.sel.success().head, "div");
